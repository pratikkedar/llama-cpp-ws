...
inline static void ggml_vec_relu_q8_0 (const int n, int8_t * y, const int8_t * x) { for (int i = 0; i < n; ++i) y[i] = (x[i] > 0) ? x[i] : 0; }
...
static void ggml_gemm_F32(const struct ggml_tensor * a, const struct ggml_tensor * b, struct ggml_tensor * c) 
{
    ...
    for (int idx = 0; idx < n; idx++) {
        ...
        int64_t offset_b = idx * K * N;
        int64_t offset_result = idx * M * N;

        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
                float sum = 0.0f;
                for (int k = 0; k < K; k++) {
                    sum += a_data[offset_a + i * K + k] * b_data[offset_b + k * N + j];
                }
                result_data[offset_result + i * N + j] = sum;
            }
        }
    }
}
    ...
    for (int idx = 0; idx < n; idx++) {
        ...
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
                int8_t sum = 0;
                for (int k = 0; k < K; k++) {
                    // printf("a : %d  b : %d", a_data[offset_a + i * K + k], b_data[offset_b + k * N + j]);
                    sum += a_data[offset_a + i * K + k] * b_data[offset_b + k * N + j];
                }
                result_data[offset_result + i * N + j] = sum;
            }
...
struct ggml_tensor * ggml_gemm( struct ggml_context * ctx, struct ggml_tensor * a, struct ggml_tensor * b) 
{    
    // Ensuring that inputs tensor are of either float or q8_0 type, and should be same to each other
    GGML_ASSERT((a->type == GGML_TYPE_F32 && b->type == GGML_TYPE_F32) || (a->type == GGML_TYPE_Q8_0 && b->type == GGML_TYPE_Q8_0) );

    // Ensure that both tensors are compatible for mul operation
...
// ggml_compute_forward_relu_q8_0

static void ggml_compute_forward_relu_q8_0(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    for (int i = 0; i < n; i++) {
        ggml_vec_relu_q8_0(nc,
                (int8_t *) ((char *) dst->data  + i*( dst->nb[1])),
                (int8_t *) ((char *) src0->data + i*(src0->nb[1])));
    }
}
...
static void ggml_compute_forward_relu(
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_Q8_0:
            {
                ggml_compute_forward_relu_q8_0(params, dst);
            } break;   
        default:
            {
                GGML_ASSERT(false);
